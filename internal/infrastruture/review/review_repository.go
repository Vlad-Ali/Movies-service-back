...
func (r *ReviewRepository) GetReviewsByMovie(ctx context.Context, movieID object2.MovieID) ([]*reviewdomain.ReviewInfo, error) {
	...
	query := `SELECT (SELECT u.username FROM users AS u WHERE u.id = r.user_id), r.text, r.writing_date, COALESCE((SELECT um.user_rating FROM user_movies AS um
              WHERE um.user_id = r.user_id AND um.movie_id = r.movie_id), 0) FROM reviews AS r
              WHERE r.movie_id = $1`
	...
	for rows.Next() {
		...
		var date time.Time
		err = rows.Scan(&reviewInfo.Username, &reviewInfo.Text, &date, &reviewInfo.UserRating)
